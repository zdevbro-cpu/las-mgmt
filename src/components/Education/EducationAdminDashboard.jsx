  ...
  useEffect(() => {
    if (selectedEducation && isScannerOpen) {
      ...
      const onScanSuccess = async (decodedText) => {
        // Pause scanner slightly maybe? Optional.
        const app = applications.find(a => a.qr_code_data === decodedText || a.id === decodedText);
        if (!app) {
          ...
          <div className="flex items-center justify-between gap-4 mb-6">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 font-bold rounded-lg transition-colors ${
                isScannerOpen ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-[#e0f2f1] text-[#249689] hover:bg-[#b2dfdb]'
              }`}
            >
              {isScannerOpen ? (
                <><X size={20} /> 스캐너 닫기</>
