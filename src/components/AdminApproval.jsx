...
import { Edit2, Trash2, LogOut, Check, X } from 'lucide-react'
import { supabase } from '../lib/supabase'

export default function AdminApproval({ user, onLogout }) {
  ...
  const loadSales = async () => {
    try {
      ...
      console.log('구매자 데이터 로드 성공:', data)
      console.log('구매자 수:', data?.length || 0)
      setSales(data || [])
    } catch (err) {
  ...
  const filteredSales = sales.filter(s => 
    ...
    s.birth_date?.includes(customerSearchTerm) ||
    s.email?.toLowerCase().includes(customerSearchTerm.toLowerCase()) ||
    s.phone?.includes(customerSearchTerm)
  )
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="bg-gray-50 px-6 py-4 border-b">
            <h2 className="font-bold text-xl" style={{ color: '#249689' }}>회원관리</h2>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="bg-gray-50 px-6 py-4 border-b">
            <h2 className="font-bold text-xl" style={{ color: '#249689' }}>구매자관리</h2>
          </div>
          ...
          <div className="p-4 border-b flex items-center gap-4">
            ...
            <input
              ...
              placeholder="이름, 생년월일, 이메일, 전화번호로 검색"
              className="flex-1 px-4 py-2 border border-gray-300"
              style={{ borderRadius: '10px', fontSize: '15px' }}
            />
            <button 
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block mb-2 font-bold" style={{ fontSize: '15px' }}>생년월일</label>
                <input
                  type="date"
                  name="birth_date"
