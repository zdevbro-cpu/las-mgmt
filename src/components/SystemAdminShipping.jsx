...
export default function SystemAdminShipping({ user, onNavigate }) {
  const config = {
    ...
    renderTableRow: (purchase, formatDate, formatPhoneNumber) => (
      <>
        <td className="px-3 py-3 whitespace-nowrap" style={{ fontSize: '15px', width: '110px' }}>
          {formatDate(purchase.created_at)}
        </td>
        <td className="px-3 py-3 whitespace-nowrap font-bold" style={{ fontSize: '15px', color: '#249689', width: '120px' }}>
          {purchase.branch_name || '-'}
        </td>
        <td className="px-3 py-3 whitespace-nowrap" style={{ fontSize: '15px', width: '100px' }}>
          {purchase.customer_name || '-'}
        </td>
        <td className="px-3 py-3 whitespace-nowrap" style={{ fontSize: '15px', width: '130px' }}>
          {formatPhoneNumber(purchase.customer_phone || purchase.phone)}
        </td>
        <td className="px-3 py-3" style={{ fontSize: '15px', width: '280px', maxWidth: '280px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {purchase.address || '-'}
        </td>
        <td className="px-3 py-3 text-center font-bold whitespace-nowrap" style={{ fontSize: '15px', width: '80px' }}>
          {purchase.quantity || 0}개
        </td>
        <td className="px-3 py-3" style={{ fontSize: '15px', width: '200px', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
