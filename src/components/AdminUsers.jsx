...
import { Edit2, Trash2, Check, X, ArrowLeft, Key, AlertCircle, Shield } from 'lucide-react'
...
export default function AdminUsers({ user, onNavigate }) {
  ...
  // 상위관리자 여부 확인
  const isSuperAdmin = user?.user_type === '상위관리자'
              ...
              {isSuperAdmin && (
                <div>
                  <label className="block mb-2 font-bold" style={{ fontSize: '15px' }}>
                    권한 <Shield size={16} className="inline ml-1" style={{ color: '#8b5cf6' }} />
                  </label>
                  <select
                    name="user_type"
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    💡 지점관리자는 해당 지점의 모든 데이터를 관리할 수 있습니다
                  </p>
                </div>
              )}
            </div>
